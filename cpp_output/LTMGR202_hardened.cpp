...
std::string VAR_281; // Auto-declared by LAZARUS healer

std::string VAR_276; // Auto-declared by LAZARUS healer

std::string VAR_271; // Auto-declared by LAZARUS healer
...
FixedString<50> bill_npi10;
FixedString<50> cbsax_table_from_user;
FixedString<50> eof_sw;
FixedString<8> idx_diag;
FixedString<8> idx_proc;
FixedString<50> ipps_cbsax_table_from_user;
FixedString<50> ltopn202;
FixedString<50> msax_table_from_user;
FixedString<50> operline_ctr;
FixedString<50> opr_stat;
FixedString<50> ppmgr_version;
...
FixedString<50> prtoper_line;
FixedString<50> prt_alos;
FixedString<50> prt_drg_adj_pay;
FixedString<50> prt_fac_cost;
FixedString<50> prt_fac_spec_rate;
FixedString<50> prt_los;
FixedString<50> prt_outlier_pay;
FixedString<50> prt_out_thresh;
FixedString<50> prt_prov;
FixedString<50> prt_rel_wt;
FixedString<50> prt_tot_pay;
FixedString<50> prt_wage_index;
int RETURN_CODE = 0;
FixedString<50> ut1_stat;
FixedString<50> ut_s_prtoper;
FixedString<50> ut_s_sysut1;
FixedString<50> w_p_new_npi10;
...
void p_0100_process_records();
void p_0100_exit();
void p_1000_calc_payment();
void p_1000_exit();
void p_1100_write_sysut2();
void p_1100_exit();
void p_1200_pps_headings();
void p_1200_exit();
void p_0000_mainline();
...
void p_0100_process_records() {
    ...
    pricer_opt_vers_sw.replace(1, 5, std::string(5, '0'));
    while (true) {
        if (to_int(eof_sw) == 1) break;
        p_0100_process_records();
    }
    // CLOSE BILLFILE
    if (file_status != "EXCEPTION_0") {
    }
    ...
    if (file_status != "EXCEPTION_0") {
        ...
        bill_npi10 = VAR_18.substr(0, 422);
    }
    bill_new_data = bill_npi10.substr(0, 422);
    if (to_int(eof_sw) == 0) {
        p_1000_calc_payment();
        p_1100_write_sysut2();
    }
...
void p_1100_write_sysut2() {
    if (to_int(operline_ctr) > 54) {
        p_1200_pps_headings();
    }
    prt_wage_index = std::string(133, ' ');
    prt_wage_index.replace(1, 6, bill_new_data.substr(10, 6));
    prt_wage_index.replace(103, 3, pps_outlier_threshold.substr(106, 3));
    prt_rel_wt = pps_relative_wgt;
    prt_fac_cost = pps_fac_costs;
    prt_alos = pps_avg_los;
    prt_los = pps_los;
    prt_tot_pay = pps_final_pay_amt;
    prt_out_thresh = pps_outlier_threshold;
    prt_drg_adj_pay = pps_drg_adj_pay_amt;
    prt_outlier_pay = pps_outlier_pay_amt;
    prt_wage_index.replace(98, 3, pps_outlier_threshold.substr(103, 3));
    prt_fac_spec_rate = pps_new_fac_spec_rate;
    prt_wage_index.replace(115, 2, pps_outlier_threshold.substr(0, 2));
    prt_wage_index = pps_wage_index;
    if (pps_outlier_threshold.substr(0, 2) == "67") {
        prt_out_thresh = pps_chrg_threshold;
    }
    prtoper_line = prt_wage_index.substr(0, 133);
    // WRITE PRTOPER
    if (file_status != "EXCEPTION_0") {
    }
    if (false /* TODO: (*(opr_stat) - '0') > 0 */) {
        std::cout << " BAD4 WRITE ON PRTOPER FILE" << std::endl;
    }
    operline_ctr = to_string(to_int(operline_ctr) + 1);
}
...
void p_1200_pps_headings() {
    prtoper_line = VAR_271.substr(0, 133);
    // WRITE PRTOPER
    if (file_status != "EXCEPTION_0") {
    }
    if (false /* TODO: (*(opr_stat) - '0') > 0 */) {
        std::cout << " BAD5 WRITE ON PRTOPER FILE" << std::endl;
    }
    prtoper_line = VAR_276.substr(0, 133);
    // WRITE PRTOPER
    if (file_status != "EXCEPTION_0") {
    }
    if (false /* TODO: (*(opr_stat) - '0') > 0 */) {
        std::cout << " BAD7 WRITE ON PRTOPER FILE" << std::endl;
    }
    prtoper_line = VAR_281.substr(0, 133);
    // WRITE PRTOPER
    if (file_status != "EXCEPTION_0") {
    }
    if (false /* TODO: (*(opr_stat) - '0') > 0 */) {
        std::cout << " BAD8 WRITE ON PRTOPER FILE" << std::endl;
    }
    prtoper_line = "  -";
    // WRITE PRTOPER
    if (file_status != "EXCEPTION_0") {
    ...
    if (false /* TODO: (*(opr_stat) - '0') > 0 */) {
        std::cout << " BAD9 WRITE ON PRTOPER FILE" << std::endl;
    }
    operline_ctr = "04";
}
