...
std::string VAR_226; // Auto-declared by LAZARUS healer

std::string VAR_55; // Auto-declared by LAZARUS healer

std::string VAR_165; // Auto-declared by LAZARUS healer
...
void p_0100_initial_routine() {
    VAR_165 = std::string(2, '0');
    VAR_165.replace(2, 9, std::string(9, ' '));
    VAR_165.replace(11, 15, std::string(15, '0'));
    VAR_165.replace(26, 11, std::string(11, ' '));
    VAR_165.replace(37, 18, std::string(18, '0'));
    VAR_165.replace(57, 50, std::string(50, '0'));
    VAR_165.replace(108, 6, std::string(6, ' '));
    VAR_165.replace(138, 54, std::string(54, '0'));
    VAR_165.replace(192, 138, std::string(138, '0'));
    VAR_55.replace(229, 23, std::string(23, '0'));
    VAR_55.at(253) = ' ';
    VAR_55.replace(255, 2, std::string(2, '0'));
    VAR_55.replace(260, 91, std::string(91, '0'));
    VAR_55.replace(369, 80, std::string(80, '0'));
    h_drug_addon = std::string(111, '0');
    VAR_165 = std::string(2, '0');
    VAR_165.replace(26, 5, cal_version.substr(0, 5));
    if (VAR_55.substr(86, 2) == "40" || VAR_55.substr(86, 2) == "05") {
        VAR_165.replace(57, 6, nat_labor_pct.substr(0, 6));
        VAR_165.replace(63, 6, nat_nonlabor_pct.substr(0, 6));
        h_drug_addon.replace(51, 6, hosp_based_pmt_rate.substr(0, 6));
        h_drug_addon.replace(27, 9, std::string(9, '0'));
        VAR_55.replace(263, 6, VAR_226.substr(12, 6));
        // MOVE to computed field
    } else {
        if (VAR_55.substr(86, 2) == "41") {
            VAR_165.replace(57, 6, nat_labor_pct.substr(0, 6));
            VAR_165.replace(63, 6, nat_nonlabor_pct.substr(0, 6));
            h_drug_addon.replace(51, 6, indp_esrd_fac_pmt_rate.substr(0, 6));
            h_drug_addon.replace(27, 9, std::string(9, '0'));
            VAR_55.replace(263, 6, VAR_226.substr(18, 6));
            // MOVE to computed field
        } else {
            VAR_165 = "52";
            VAR_165.replace(11, 6, std::string(6, '0'));
        }
    }
    // MOVE to computed field
    VAR_165.replace(83, 5, case_mix_bdgt_neut_factor.substr(0, 5));
    h_drug_addon.replace(106, 5, drug_addon.substr(0, 5));
}

void p_1000_edit_the_bill_info() {
    if (to_int(VAR_165) == 0) {
        if (VAR_55.at(85) != '1' && VAR_55.at(85) != ' ') {
            VAR_165 = "53";
        }
    }
    if (to_int(VAR_165) == 0) {
        if (false /* TODO: memcmp (VAR_55 + 32, COB_ZEROES_ALPHABETIC, 8) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_69, VAR_55 + 32)) */) {
            VAR_165 = "54";
        }
    }
    if (to_int(VAR_165) == 0) {
        if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_63, VAR_55 + 15), 0LL) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_63, VAR_55 + 15)) */) {
            VAR_165 = std::string(2, static_cast<char>(53));
        }
    }
    if (to_int(VAR_165) == 0) {
        if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_61, VAR_55 + 6), 0LL) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_61, VAR_55 + 6)) */) {
            VAR_165 = "56";
        }
    }
    if (to_int(VAR_165) == 0) {
        if (VAR_55.substr(2, 4) == "0821" || VAR_55.substr(2, 4) == "0831" || VAR_55.substr(2, 4) == "0841" || VAR_55.substr(2, 4) == "0851" || VAR_55.substr(2, 4) == "0880" || VAR_55.substr(2, 4) == "0881") {
        } else {
            VAR_165 = "57";
        }
    }
    if (to_int(VAR_165) == 0) {
        if (VAR_55.substr(0, 2) != "73" && VAR_55.substr(0, 2) != "74" && VAR_55.substr(0, 2) != "") {
            VAR_165 = "58";
        }
    }
    if (to_int(VAR_165) == 0) {
...
void p_3000_calc_payment() {
    ...
    VAR_55.replace(307, 9, h_drug_addon.substr(0, 9));
    VAR_55.replace(252, 8, std::string(8, ' '));
    if (VAR_55.substr(0, 2) == "73" && VAR_55.substr(2, 4) == "0821" || VAR_55.substr(2, 4) == "0831" || VAR_55.substr(2, 4) == "0851") {
        h_pymt_amt = to_string(to_num(0) + to_num(1));
        VAR_55.at(253) = 'A';
        VAR_55.replace(255, 2, hemo_peri_ccpd_amt.substr(0, 2));
    } else {
        if (VAR_55.substr(0, 2) == "73" && VAR_55.substr(2, 4) == "0841") {
            h_pymt_amt = to_string(to_num(0) + to_num(1));
            VAR_55.at(253) = 'A';
            VAR_55.replace(255, 2, capd_amt.substr(0, 2));
        } else {
            if (VAR_55.substr(0, 2) == "74" && VAR_55.substr(2, 4) == "0841" || VAR_55.substr(2, 4) == "0851") {
                h_pymt_amt = to_string(to_num(0) * to_num(1));
                // MOVE to computed field
            } else {
                VAR_55.at(253) = 'A';
    ...
    VAR_165.replace(17, 9, h_drug_addon.substr(0, 9));
    // MOVE to computed field
}

void p_9000_move_results() {
    if (to_int(VAR_165) < 50) {
        VAR_165.replace(31, 2, VAR_55.substr(0, 2));
        VAR_165.replace(33, 4, VAR_55.substr(2, 4));
        VAR_165.replace(2, 4, VAR_55.substr(76, 4));
        VAR_165.replace(6, 5, VAR_55.substr(80, 5));
        VAR_165.replace(69, 4, h_drug_addon.substr(78, 4));
        VAR_165.replace(73, 5, h_drug_addon.substr(82, 5));
        VAR_165.replace(78, 5, h_drug_addon.substr(87, 5));
        if (VAR_55.at(122) == 'T') {
            VAR_55.replace(229, 5, h_drug_addon.substr(106, 5));
            // MOVE to computed field
            // MOVE to computed field
            VAR_55.replace(298, 3, cbsa_blend_pct.substr(0, 3));
            VAR_55.replace(295, 3, msa_blend_pct.substr(0, 3));
            VAR_55.replace(246, 6, h_drug_addon.substr(51, 6));
            VAR_55.replace(260, 3, h_drug_addon.substr(75, 3));
            // MOVE to computed field
            VAR_55.replace(275, 6, nat_labor_pct.substr(0, 6));
        }
    } else {
        if (VAR_55.at(122) == 'T') {
            VAR_165.replace(31, 2, std::string(2, ' '));
            VAR_165.replace(33, 4, std::string(4, ' '));
            VAR_165.replace(2, 4, std::string(4, ' '));
            VAR_165.replace(6, 5, std::string(5, ' '));
            VAR_165.replace(69, 4, std::string(4, '0'));
            VAR_165.replace(73, 5, std::string(5, '0'));
            VAR_165.replace(78, 5, std::string(5, '0'));
            VAR_55.replace(229, 5, std::string(5, '0'));
            VAR_55.replace(234, 6, std::string(6, '0'));
            VAR_55.replace(298, 3, std::string(3, '0'));
            VAR_55.replace(295, 3, std::string(3, '0'));
            VAR_55.replace(307, 9, std::string(9, '0'));
            VAR_55.replace(246, 6, std::string(6, '0'));
            VAR_55.replace(252, 8, std::string(8, '0'));
            VAR_55.replace(260, 3, std::string(3, '0'));
        }
    }
}
