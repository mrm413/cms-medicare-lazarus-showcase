...
std::string VAR_304; // Auto-declared by LAZARUS healer

std::string VAR_222; // Auto-declared by LAZARUS healer

std::string VAR_166; // Auto-declared by LAZARUS healer
...
void p_0100_initial_routine() {
    VAR_184 = std::string(2, '0');
    VAR_184.replace(11, 4, std::string(4, ' '));
    VAR_184.replace(15, 80, std::string(80, '0'));
    VAR_184.replace(95, 8, std::string(8, ' '));
    VAR_184.replace(103, 11, std::string(11, '0'));
    VAR_184.replace(118, 23, std::string(23, '0'));
    h_fixed_loss_amt = std::string(79, '0');
    VAR_184.replace(118, 6, "072885");
    VAR_184.replace(124, 6, "027115");
    VAR_184.replace(130, 7, "3495615");
    h_fixed_loss_amt = "24450";
    VAR_184.replace(137, 4, "0934");
}
...
void p_1000_edit_the_bill_info() {
    if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_173, VAR_166 + 21)) */ && to_int(VAR_166) > 0) {
        h_fixed_loss_amt = VAR_166.substr(21, 3);
    } else {
        VAR_184 = "56";
    }
    if (to_int(VAR_184) == 0) {
    ...
    if (to_int(VAR_184) == 0) {
        if (VAR_166.substr(29, 8) < VAR_222.substr(16, 8) || VAR_166.substr(29, 8) < VAR_304.substr(4, 8)) {
            VAR_184 = std::string(2, static_cast<char>(53));
        }
    }
    if (to_int(VAR_184) == 0) {
        if (VAR_222.substr(40, 8) > "00000000") {
            if (VAR_166.substr(29, 8) >= VAR_222.substr(40, 8)) {
                VAR_184 = "51";
            }
        }
    }
    if (to_int(VAR_184) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_181, VAR_166 + 37)) */) {
            VAR_184 = "58";
        }
    }
    if (to_int(VAR_184) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_175, VAR_166 + 27)) */ || to_int(VAR_166) > 60) {
            VAR_184 = "61";
        }
    }
    if (to_int(VAR_184) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_174, VAR_166 + 24)) */ || to_int(VAR_166) == 0 && to_int(h_fixed_loss_amt) > 0) {
            VAR_184 = "62";
        }
    }
    if (to_int(VAR_184) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_166 + 27, 2, cob_get_llint (COB_SET_DATA (f_174, VAR_166 + 24)), 0) > 0 */) {
            VAR_184 = "62";
        }
    }
    if (to_int(VAR_184) == 0) {
...
void p_1200_days_used() {
    if (to_int(VAR_166) > 0 && to_int(h_fixed_loss_amt) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_166 + 27, 2, cob_get_llint (h_los), 0) > 0 */) {
            VAR_184.replace(106, 3, h_fixed_loss_amt.substr(0, 3));
        } else {
            // MOVE to computed field
        }
    } else {
        if (to_int(h_fixed_loss_amt) > 0 && to_int(VAR_166) == 0) {
            if (h_fixed_loss_amt.substr(3, 3) > h_fixed_loss_amt.substr(0, 3)) {
                VAR_184.replace(103, 3, h_fixed_loss_amt.substr(0, 3));
            } else {
                VAR_184.replace(103, 3, h_fixed_loss_amt.substr(3, 3));
            }
        } else {
            if (to_int(h_fixed_loss_amt) > 0 && to_int(VAR_166) > 0) {
                if (h_fixed_loss_amt.substr(3, 3) > h_fixed_loss_amt.substr(0, 3)) {
                    VAR_184.replace(103, 3, h_fixed_loss_amt.substr(0, 3));
                    VAR_184.replace(106, 3, std::string(3, '0'));
                } else {
                    if (false /* TODO: cob_cmp_numdisp (h_fixed_loss_amt + 6, 5, cob_get_llint (h_los), 0) > 0 */) {
                        VAR_184.replace(103, 3, h_fixed_loss_amt.substr(3, 3));
                    } else {
                        if (false /* TODO: cob_cmp_numdisp (h_fixed_loss_amt + 6, 5, cob_get_llint (h_los), 0) <= 0 */) {
                            VAR_184.replace(103, 3, h_fixed_loss_amt.substr(3, 3));
...
void p_1700_edit_drg_code() {
    VAR_184.replace(95, 3, VAR_166.substr(18, 3));
    if (to_int(VAR_184) == 0) {
        while (true) {
            ...
            if (false /* TODO: (ret = memcmp (VAR_19 + 11LL * ((cob_s64_t)(wwm_indx) - 1), VAR_184 + 95, 3)) == 0 */) {
                p_1750_find_value();
                break;
            }
            // if (ret < 0) head = b_148;
...
void p_2000_assemble_pps_variables() {
    if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_307, VAR_304 + 12)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_307, VAR_304 + 12), 0LL) > 0 */) {
        // MOVE to computed field
    } else {
        VAR_184 = "52";
    }
    if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_279, VAR_222 + 101)) */) {
        VAR_184 = "65";
    }
    // MOVE to computed field
    if (to_int(VAR_184) > 0 && to_int(VAR_184) < 6) {
    } else {
        VAR_184 = "72";
    }
    h_blend_fac = "0";
    h_blend_pps = "1";
    h_fixed_loss_amt.replace(13, 2, std::string(2, '0'));
    if (to_int(VAR_184) == 1) {
        h_fixed_loss_amt.replace(15, 2, "08");
        h_fixed_loss_amt.replace(17, 2, "02");
        h_fixed_loss_amt.replace(13, 2, "04");
    } else {
        if (to_int(VAR_184) == 2) {
            h_fixed_loss_amt.replace(15, 2, "06");
            h_fixed_loss_amt.replace(17, 2, "04");
            h_fixed_loss_amt.replace(13, 2, "08");
        } else {
            if (to_int(VAR_184) == 3) {
                h_fixed_loss_amt.replace(15, 2, "04");
                h_fixed_loss_amt.replace(17, 2, "06");
                h_fixed_loss_amt.replace(13, 2, "12");
            } else {
                if (to_int(VAR_184) == 4) {
                    h_fixed_loss_amt.replace(15, 2, "02");
                    h_fixed_loss_amt.replace(17, 2, "08");
                    h_fixed_loss_amt.replace(13, 2, "16");
                }
            }
        }
    }
...
void p_3000_calc_payment() {
    VAR_184.replace(110, 4, VAR_222.substr(87, 4));
    ...
    if (false /* TODO: cob_cmp_numdisp (h_fixed_loss_amt, 3, cob_get_llint (h_ssot), 0) <= 0 */) {
        p_3400_short_stay();
    }
}
...
void p_3400_short_stay() {
    ...
    if (h_fixed_loss_amt.substr(28, 9) < h_fixed_loss_amt.substr(19, 9)) {
        if (h_fixed_loss_amt.substr(28, 9) < VAR_184.substr(41, 9)) {
            VAR_184.replace(41, 9, h_fixed_loss_amt.substr(28, 9));
            VAR_184 = "02";
        } else {
        }
    } else {
        if (h_fixed_loss_amt.substr(19, 9) < VAR_184.substr(41, 9)) {
            VAR_184.replace(41, 9, h_fixed_loss_amt.substr(19, 9));
            VAR_184 = "02";
        } else {
        }
    }
...
void p_7000_calc_outlier() {
    if (VAR_184.substr(68, 9) > VAR_184.substr(86, 9)) {
    }
    if (VAR_166.at(46) == '1') {
        // MOVE "0" to computed field
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_192, VAR_184 + 29), 0LL) > 0 */ && to_int(VAR_184) == 2) {
        VAR_184 = "03";
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_192, VAR_184 + 29), 0LL) > 0 */ && to_int(VAR_184) == 0) {
        VAR_184 = "01";
    }
    if (to_int(VAR_184) == 0 || to_int(VAR_184) == 2) {
        if (false /* TODO: cob_cmp_numdisp (VAR_184 + 103, 3, cob_get_llint (h_ssot), 0) > 0 */) {
            VAR_184.replace(106, 3, std::string(3, '0'));
        } else {
        }
    }
    if (to_int(VAR_184) == 1 || to_int(VAR_184) == 3) {
        if (VAR_166.substr(24, 3) < h_fixed_loss_amt.substr(0, 3) || VAR_184.at(161) == 'Y') {
            VAR_184 = "67";
        } else {
        }
    } else {
...
void p_9000_move_results() {
    if (to_int(VAR_184) < 50) {
        VAR_184.replace(38, 3, h_fixed_loss_amt.substr(0, 3));
        VAR_184.replace(98, 5, "V03.2");
    } else {
        VAR_184.replace(11, 4, std::string(4, ' '));
        VAR_184.replace(15, 80, std::string(80, '0'));
        VAR_184.replace(95, 8, std::string(8, ' '));
