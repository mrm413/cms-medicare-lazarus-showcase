...
FixedString<50> cbsa_wage_index_record;
FixedString<100> cbsa_wir_area_wageind;
FixedString<50> hold_snf_cbsa_1st;
FixedString<50> hold_variables;
FixedString<50> ma1;
FixedString<50> ma2;
FixedString<50> ma3;
int RETURN_CODE = 0;
FixedString<50> snfdr_version;
FixedString<50> snfpr205;
FixedString<50> snfpr210;
FixedString<80> snf_data;
FixedString<50> snf_rtc;
FixedString<50> snf_spec_wi_x;
FixedString<50> snf_supp_wi;
FixedString<50> ws_wi_pct_adj_fy2021;
FixedString<50> ws_wi_pct_decrease_fy2021;
...
void p_0100_exit();
void p_1900_select_cbsa_wage_index();
void p_1900_exit();
void p_2000_read_cbsa_file();
void p_2000_exit();
void p_2010_select_current_wage_index();
void p_2010_exit();
void p_2020_cap_wage_index_decrease();
void p_2020_exit();
void p_0000_mainline();

void p_0100_process_records() {
    p_0100_process_records();
    snf_data.replace(220, 10, std::string(10, '0'));
    cbsa_wir_area_wageind = std::string(19, '0');
    hold_variables = std::string(17, '0');
    if (snf_data.substr(29, 8) < "19980701") {
        snf_data.replace(228, 2, "40");
    }
    hold_snf_cbsa_1st = snf_data.substr(4, 5);
    if (snf_data.substr(4, 3) == "" || snf_data.substr(4, 3) == "999") {
        hold_snf_cbsa_1st = std::string(3, ' ');
    }
    if (snf_data.substr(4, 3) == "" || snf_data.substr(4, 3) == "999") {
        hold_variables.at(0) = static_cast<char>(82);
    } else {
        hold_variables.at(0) = static_cast<char>(85);
    }
    p_1900_select_cbsa_wage_index();
    if (false /* cob_cmp != 0 */) {
    }
    if (snf_data.substr(29, 8) > "20200930") {
    }
    if (snf_data.substr(29, 8) > "20190930" && snf_data.substr(29, 8) < "20201001") {
    }
}
...
void p_1900_select_cbsa_wage_index() {
    ...
    if (snf_data.at(9) == '1' && false /* TODO: !cob_is_numeric (COB_SET_DATA (f_40, snf_data + 10)) */) {
        snf_data.replace(228, 2, "30");
    }
    if (snf_data.at(9) == '1' && false /* TODO: cob_is_numeric (COB_SET_DATA (f_40, snf_data + 10)) */ && false /* cob_cmp > 0 */) {
        cbsa_wir_area_wageind = snf_data.substr(4, 5);
        cbsa_wir_area_wageind.replace(5, 8, "20051001");
        cbsa_wir_area_wageind.replace(13, 6, snf_data.substr(10, 6));
    }
    p_2000_read_cbsa_file();
    if (false /* cob_cmp != 0 */) {
    }
    // ma2 = b_108;
    while (true) {
        if (false /* TODO: memcmp (VAR_106 + 19LL * ((cob_s64_t)(ma2) - 1), hold_snf_cbsa_1st, 5) != 0 */) break;
        p_2010_select_current_wage_index();
        // ma2 = (b_109 + 1);
    }
    if (to_int(cbsa_wir_area_wageind) <= 0) {
        snf_data.replace(228, 2, "30");
    }
    if (snf_data.substr(29, 8) > "20200930" && snf_data.at(255) == '1' && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_95, snf_data + 256), 0LL) <= 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_95, snf_data + 256)) */) {
        snf_data.replace(228, 2, "30");
    }
    if (snf_data.substr(29, 8) > "20200930" && snf_data.at(255) == '1' && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_95, snf_data + 256), 0LL) > 0 */) {
        p_2020_cap_wage_index_decrease();
    }
}
...
void p_2000_read_cbsa_file() {
    ma1 = to_string(1);
    // UNHANDLED: const int max = 8000;
    while (true) {
        if (false /* TODO: ma1 > max */) {
            snf_data.replace(228, 2, "30");
        }
        if (false /* TODO: memcmp (VAR_106 + 19LL * ((cob_s64_t)(ma1) - 1), hold_snf_cbsa_1st, 5) == 0 */) {
            // ma2 = b_108;
            break;
        }
        // UNHANDLED: b_108++;
...
void p_2010_select_current_wage_index() {
    if (false /* TODO: memcmp (snf_data + 29, VAR_106 + 5 + 19LL * ((cob_s64_t)(ma2) - 1), 8) >= 0 */) {
    }
}
...
void p_2020_cap_wage_index_decrease() {
    if (false /* decimal_cmp < 0 */) {
        cbsa_wir_area_wageind = to_string(to_num(0) * to_num(1));
    }
}
