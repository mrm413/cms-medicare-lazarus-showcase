...
std::string VAR_305; // Auto-declared by LAZARUS healer

std::string VAR_223; // Auto-declared by LAZARUS healer
...
void p_0100_initial_routine() {
    VAR_185 = std::string(2, '0');
    VAR_185.replace(11, 4, std::string(4, ' '));
    VAR_185.replace(15, 80, std::string(80, '0'));
    VAR_185.replace(95, 8, std::string(8, ' '));
    VAR_185.replace(103, 11, std::string(11, '0'));
    VAR_185.replace(118, 23, std::string(23, '0'));
    h_los_ratio = std::string(85, '0');
    VAR_185.replace(118, 6, "072885");
    VAR_185.replace(124, 6, "027115");
    VAR_185.replace(130, 7, "3572618");
    h_fixed_loss_amt = "19590";
    VAR_185.replace(137, 4, "0940");
}
...
void p_1000_edit_the_bill_info() {
    if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_174, VAR_167 + 21)) */ && to_int(VAR_167) > 0) {
        h_los_ratio = VAR_167.substr(21, 3);
    } else {
        VAR_185 = "56";
    }
    if (to_int(VAR_185) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_277, VAR_223 + 87)) */) {
            VAR_185 = "50";
        }
    }
    if (to_int(VAR_185) == 0) {
    ...
    if (to_int(VAR_185) == 0) {
        if (VAR_167.substr(29, 8) < VAR_223.substr(16, 8) || VAR_167.substr(29, 8) < VAR_305.substr(4, 8)) {
            VAR_185 = std::string(2, static_cast<char>(53));
        }
    }
    if (to_int(VAR_185) == 0) {
        if (VAR_223.substr(40, 8) > "00000000") {
            if (VAR_167.substr(29, 8) >= VAR_223.substr(40, 8)) {
                VAR_185 = "51";
            }
        }
    }
    if (to_int(VAR_185) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_182, VAR_167 + 37)) */) {
            VAR_185 = "58";
        }
    }
    if (to_int(VAR_185) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_176, VAR_167 + 27)) */ || to_int(VAR_167) > 60) {
            VAR_185 = "61";
        }
    }
    if (to_int(VAR_185) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_175, VAR_167 + 24)) */ || to_int(VAR_167) == 0 && to_int(h_los_ratio) > 0) {
            VAR_185 = "62";
        }
    }
    if (to_int(VAR_185) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_167 + 27, 2, cob_get_llint (COB_SET_DATA (f_175, VAR_167 + 24)), 0) > 0 */) {
            VAR_185 = "62";
        }
    }
    if (to_int(VAR_185) == 0) {
...
void p_1200_days_used() {
    if (to_int(VAR_167) > 0 && to_int(h_los_ratio) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_167 + 27, 2, cob_get_llint (h_los), 0) > 0 */) {
            VAR_185.replace(106, 3, h_los_ratio.substr(0, 3));
        } else {
            // MOVE to computed field
        }
    } else {
        if (to_int(h_los_ratio) > 0 && to_int(VAR_167) == 0) {
            if (h_los_ratio.substr(3, 3) > h_los_ratio.substr(0, 3)) {
                VAR_185.replace(103, 3, h_los_ratio.substr(0, 3));
            } else {
                VAR_185.replace(103, 3, h_los_ratio.substr(3, 3));
            }
        } else {
            if (to_int(h_los_ratio) > 0 && to_int(VAR_167) > 0) {
                if (h_los_ratio.substr(3, 3) > h_los_ratio.substr(0, 3)) {
                    VAR_185.replace(103, 3, h_los_ratio.substr(0, 3));
                    VAR_185.replace(106, 3, std::string(3, '0'));
                } else {
...
void p_1700_edit_drg_code() {
    VAR_185.replace(95, 3, VAR_167.substr(18, 3));
    if (to_int(VAR_185) == 0) {
        while (true) {
            ...
            if (false /* TODO: (ret = memcmp (VAR_19 + 11LL * ((cob_s64_t)(wwm_indx) - 1), VAR_185 + 95, 3)) == 0 */) {
                p_1750_find_value();
                break;
            }
            // if (ret < 0) head = b_148;
...
void p_2000_assemble_pps_variables() {
    if (VAR_223.substr(24, 8) >= "20031001" && VAR_167.substr(29, 8) >= VAR_223.substr(24, 8)) {
        if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_309, VAR_305 + 18)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_309, VAR_305 + 18), 0LL) > 0 */) {
            // MOVE to computed field
        } else {
            VAR_185 = "52";
        }
    } else {
        if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_308, VAR_305 + 12)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_308, VAR_305 + 12), 0LL) > 0 */) {
            // MOVE to computed field
        } else {
            VAR_185 = "52";
        }
    }
    if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_280, VAR_223 + 101)) */) {
        VAR_185 = "65";
    }
    // MOVE to computed field
    if (to_int(VAR_185) > 0 && to_int(VAR_185) < 6) {
    } else {
        VAR_185 = "72";
    }
    h_blend_fac = "0";
...
void p_3000_calc_payment() {
    VAR_185.replace(110, 4, VAR_223.substr(87, 4));
...
void p_3400_short_stay() {
    ...
    if (h_los_ratio.substr(28, 9) < h_los_ratio.substr(19, 9)) {
        if (h_los_ratio.substr(28, 9) < VAR_185.substr(41, 9)) {
            VAR_185.replace(41, 9, h_los_ratio.substr(28, 9));
            VAR_185 = "02";
        } else {
        }
    } else {
        if (h_los_ratio.substr(19, 9) < VAR_185.substr(41, 9)) {
            VAR_185.replace(41, 9, h_los_ratio.substr(19, 9));
            VAR_185 = "02";
        } else {
        }
    }
...
void p_4000_special_provider() {
    if (VAR_167.substr(29, 8) >= "20030701" && VAR_167.substr(29, 8) < "20040101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    } else {
        if (VAR_167.substr(29, 8) >= "20040101" && VAR_167.substr(29, 8) < "20050101") {
            h_ss_cost = to_string(to_num(0) * to_num(1));
            h_ss_pay_amt = to_string(to_num(0) * to_num(1));
        }
    }
...
void p_7000_calc_outlier() {
    if (VAR_185.substr(68, 9) > VAR_185.substr(86, 9)) {
    }
    if (VAR_167.at(46) == '1') {
        // MOVE "0" to computed field
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_193, VAR_185 + 29), 0LL) > 0 */ && to_int(VAR_185) == 2) {
        VAR_185 = "03";
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_193, VAR_185 + 29), 0LL) > 0 */ && to_int(VAR_185) == 0) {
        VAR_185 = "01";
    }
    if (to_int(VAR_185) == 0 || to_int(VAR_185) == 2) {
        if (false /* TODO: cob_cmp_numdisp (VAR_185 + 103, 3, cob_get_llint (h_ssot), 0) > 0 */) {
            VAR_185.replace(106, 3, std::string(3, '0'));
        } else {
        }
    }
    if (to_int(VAR_185) == 1 || to_int(VAR_185) == 3) {
        if (VAR_167.substr(24, 3) < h_los_ratio.substr(0, 3) || VAR_185.at(161) == 'Y') {
            VAR_185 = "67";
        } else {
        }
    } else {
...
void p_9000_move_results() {
    if (to_int(VAR_185) < 50) {
        VAR_185.replace(38, 3, h_los_ratio.substr(0, 3));
        VAR_185.replace(98, 5, "V04.2");
    } else {
        VAR_185.replace(11, 4, std::string(4, ' '));
        VAR_185.replace(15, 80, std::string(80, '0'));
        VAR_185.replace(95, 8, std::string(8, ' '));
