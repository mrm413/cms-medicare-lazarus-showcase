...
std::string VAR_228; // Auto-declared by LAZARUS healer

std::string VAR_57; // Auto-declared by LAZARUS healer

std::string VAR_167; // Auto-declared by LAZARUS healer
...
void p_0100_initial_routine() {
    VAR_167 = std::string(2, '0');
    VAR_167.replace(2, 9, std::string(9, ' '));
    VAR_167.replace(11, 15, std::string(15, '0'));
    VAR_167.replace(26, 11, std::string(11, ' '));
    VAR_167.replace(37, 18, std::string(18, '0'));
    VAR_167.replace(57, 50, std::string(50, '0'));
    VAR_167.replace(108, 6, std::string(6, ' '));
    VAR_167.replace(138, 54, std::string(54, '0'));
    VAR_167.replace(192, 138, std::string(138, '0'));
    VAR_57.replace(229, 23, std::string(23, '0'));
    VAR_57.at(253) = ' ';
    VAR_57.replace(255, 2, std::string(2, '0'));
    VAR_57.replace(260, 91, std::string(91, '0'));
    VAR_57.replace(369, 80, std::string(80, '0'));
    h_drug_addon = std::string(120, '0');
    VAR_167 = std::string(2, '0');
    VAR_167.replace(26, 5, cal_version.substr(0, 5));
    if (VAR_57.substr(86, 2) == "40" || VAR_57.substr(86, 2) == "05") {
        VAR_167.replace(57, 6, nat_labor_pct.substr(0, 6));
        VAR_167.replace(63, 6, nat_nonlabor_pct.substr(0, 6));
        h_drug_addon.replace(60, 6, hosp_based_pmt_rate.substr(0, 6));
        h_2006_wage_adj_pymt = to_string(to_num(0) * to_num(1));
        h_drug_addon.replace(36, 9, h_drug_addon.substr(18, 9));
        VAR_57.replace(263, 6, VAR_228.substr(12, 6));
        // MOVE to computed field
    } else {
        if (VAR_57.substr(86, 2) == "41") {
            VAR_167.replace(57, 6, nat_labor_pct.substr(0, 6));
            VAR_167.replace(63, 6, nat_nonlabor_pct.substr(0, 6));
            h_drug_addon.replace(60, 6, indp_esrd_fac_pmt_rate.substr(0, 6));
            h_2006_wage_adj_pymt = to_string(to_num(0) * to_num(1));
            h_drug_addon.replace(36, 9, h_drug_addon.substr(18, 9));
            VAR_57.replace(263, 6, VAR_228.substr(18, 6));
            // MOVE to computed field
        } else {
            VAR_167 = "52";
            VAR_167.replace(11, 6, std::string(6, '0'));
        }
    }
    // MOVE to computed field
    VAR_167.replace(83, 5, case_mix_bdgt_neut_factor.substr(0, 5));
    h_drug_addon.replace(115, 5, drug_addon.substr(0, 5));
}

void p_1000_edit_the_bill_info() {
    if (to_int(VAR_167) == 0) {
        if (VAR_57.at(85) != '1' && VAR_57.at(85) != ' ') {
            VAR_167 = "53";
        }
    }
    if (to_int(VAR_167) == 0) {
        if (false /* TODO: memcmp (VAR_57 + 32, COB_ZEROES_ALPHABETIC, 8) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_71, VAR_57 + 32)) */) {
            VAR_167 = "54";
        }
    }
    if (to_int(VAR_167) == 0) {
        if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_65, VAR_57 + 15), 0LL) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_65, VAR_57 + 15)) */) {
            VAR_167 = std::string(2, static_cast<char>(53));
        }
    }
    if (to_int(VAR_167) == 0) {
        if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_63, VAR_57 + 6), 0LL) == 0 */ || false /* TODO: !cob_is_numeric (COB_SET_DATA (f_63, VAR_57 + 6)) */) {
            VAR_167 = "56";
        }
    }
    if (to_int(VAR_167) == 0) {
        if (VAR_57.substr(2, 4) == "0821" || VAR_57.substr(2, 4) == "0831" || VAR_57.substr(2, 4) == "0841" || VAR_57.substr(2, 4) == "0851" || VAR_57.substr(2, 4) == "0880" || VAR_57.substr(2, 4) == "0881") {
        } else {
            VAR_167 = "57";
        }
    }
    if (to_int(VAR_167) == 0) {
        if (VAR_57.substr(0, 2) != "73" && VAR_57.substr(0, 2) != "74" && VAR_57.substr(0, 2) != "") {
            VAR_167 = "58";
        }
    }
    if (to_int(VAR_167) == 0) {
...
void p_3000_calc_payment() {
    ...
    VAR_57.replace(307, 9, h_drug_addon.substr(0, 9));
    VAR_57.replace(252, 8, std::string(8, ' '));
    if (VAR_57.substr(0, 2) == "73" && VAR_57.substr(2, 4) == "0821" || VAR_57.substr(2, 4) == "0831" || VAR_57.substr(2, 4) == "0851") {
        h_pymt_amt = to_string(to_num(0) + to_num(1));
        VAR_57.at(253) = 'A';
        VAR_57.replace(255, 2, hemo_peri_ccpd_amt.substr(0, 2));
    } else {
        if (VAR_57.substr(0, 2) == "73" && VAR_57.substr(2, 4) == "0841") {
            h_pymt_amt = to_string(to_num(0) + to_num(1));
            VAR_57.at(253) = 'A';
            VAR_57.replace(255, 2, capd_amt.substr(0, 2));
        } else {
            if (VAR_57.substr(0, 2) == "74" && VAR_57.substr(2, 4) == "0841" || VAR_57.substr(2, 4) == "0851") {
                h_pymt_amt = to_string(to_num(0) * to_num(1));
                // MOVE to computed field
            } else {
                VAR_57.at(253) = 'A';
    ...
    VAR_167.replace(17, 9, h_drug_addon.substr(0, 9));
    // MOVE to computed field
}

void p_9000_move_results() {
    if (to_int(VAR_167) < 50) {
        VAR_167.replace(31, 2, VAR_57.substr(0, 2));
        VAR_167.replace(33, 4, VAR_57.substr(2, 4));
        VAR_167.replace(2, 4, VAR_57.substr(76, 4));
        VAR_167.replace(6, 5, VAR_57.substr(80, 5));
        VAR_167.replace(69, 4, h_drug_addon.substr(87, 4));
        VAR_167.replace(73, 5, h_drug_addon.substr(91, 5));
        VAR_167.replace(78, 5, h_drug_addon.substr(96, 5));
        if (VAR_57.at(122) == 'T') {
            VAR_57.replace(229, 5, h_drug_addon.substr(115, 5));
            // MOVE to computed field
            // MOVE to computed field
            VAR_57.replace(298, 3, cbsa_blend_pct.substr(0, 3));
            VAR_57.replace(295, 3, msa_blend_pct.substr(0, 3));
            VAR_57.replace(246, 6, h_drug_addon.substr(60, 6));
            VAR_57.replace(260, 3, h_drug_addon.substr(84, 3));
            // MOVE to computed field
            VAR_57.replace(275, 6, nat_labor_pct.substr(0, 6));
        }
    } else {
        if (VAR_57.at(122) == 'T') {
            VAR_167.replace(31, 2, std::string(2, ' '));
            VAR_167.replace(33, 4, std::string(4, ' '));
            VAR_167.replace(2, 4, std::string(4, ' '));
            VAR_167.replace(6, 5, std::string(5, ' '));
            VAR_167.replace(69, 4, std::string(4, '0'));
            VAR_167.replace(73, 5, std::string(5, '0'));
            VAR_167.replace(78, 5, std::string(5, '0'));
            VAR_57.replace(229, 5, std::string(5, '0'));
            VAR_57.replace(234, 6, std::string(6, '0'));
            VAR_57.replace(240, 6, std::string(6, '0'));
            VAR_57.replace(298, 3, std::string(3, '0'));
            VAR_57.replace(295, 3, std::string(3, '0'));
            VAR_57.replace(307, 9, std::string(9, '0'));
            VAR_57.replace(246, 6, std::string(6, '0'));
            VAR_57.replace(252, 8, std::string(8, '0'));
            VAR_57.replace(260, 3, std::string(3, '0'));
        }
    }
}
