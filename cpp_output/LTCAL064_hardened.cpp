...
std::string VAR_327; // Auto-declared by LAZARUS healer

std::string VAR_234; // Auto-declared by LAZARUS healer

std::string VAR_177; // Auto-declared by LAZARUS healer

std::string VAR_195; // Auto-declared by LAZARUS healer
...
FixedString<50> fed_fy_begin_04;
...
void p_0100_initial_routine() {
    VAR_195 = std::string(2, '0');
    VAR_195.replace(11, 4, std::string(4, ' '));
    VAR_195.replace(15, 80, std::string(80, '0'));
    VAR_195.replace(95, 8, std::string(8, ' '));
    VAR_195.replace(103, 11, std::string(11, '0'));
    VAR_195.replace(118, 23, std::string(23, '0'));
    h_los_ratio = std::string(85, '0');
    VAR_195.replace(118, 6, "072885");
    VAR_195.replace(124, 6, "027115");
    VAR_195.replace(130, 7, "3808604");
    h_fixed_loss_amt = "10501";
    VAR_195.replace(137, 4, "1000");
}
...
void p_1000_edit_the_bill_info() {
    if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_184, VAR_177 + 21)) */ && to_int(VAR_177) > 0) {
        h_los_ratio = VAR_177.substr(21, 3);
    } else {
        VAR_195 = "56";
    }
    if (to_int(VAR_195) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_288, VAR_234 + 87)) */) {
            VAR_195 = "50";
        }
    }
    if (to_int(VAR_195) == 0) {
    ...
    if (to_int(VAR_195) == 0) {
        if (VAR_177.substr(29, 8) < VAR_234.substr(16, 8) || VAR_177.substr(29, 8) < VAR_327.substr(5, 8)) {
            VAR_195 = std::string(2, static_cast<char>(53));
        }
    }
    if (to_int(VAR_195) == 0) {
        if (VAR_234.substr(40, 8) > "00000000") {
            if (VAR_177.substr(29, 8) >= VAR_234.substr(40, 8)) {
                VAR_195 = "51";
            }
        }
    }
    if (to_int(VAR_195) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_192, VAR_177 + 37)) */) {
            VAR_195 = "58";
        }
    }
    if (to_int(VAR_195) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_186, VAR_177 + 27)) */ || to_int(VAR_177) > 60) {
            VAR_195 = "61";
        }
    }
    if (to_int(VAR_195) == 0) {
        if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_185, VAR_177 + 24)) */ || to_int(VAR_177) == 0 && to_int(h_los_ratio) > 0) {
            VAR_195 = "62";
        }
    }
    if (to_int(VAR_195) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_177 + 27, 2, cob_get_llint (COB_SET_DATA (f_185, VAR_177 + 24)), 0) > 0 */) {
            VAR_195 = "62";
        }
    }
    if (to_int(VAR_195) == 0) {
...
void p_1200_days_used() {
    if (to_int(VAR_177) > 0 && to_int(h_los_ratio) == 0) {
        if (false /* TODO: cob_cmp_numdisp (VAR_177 + 27, 2, cob_get_llint (h_los), 0) > 0 */) {
            VAR_195.replace(106, 3, h_los_ratio.substr(0, 3));
        } else {
            // MOVE to computed field
        }
    } else {
        if (to_int(h_los_ratio) > 0 && to_int(VAR_177) == 0) {
            if (h_los_ratio.substr(3, 3) > h_los_ratio.substr(0, 3)) {
                VAR_195.replace(103, 3, h_los_ratio.substr(0, 3));
            } else {
                VAR_195.replace(103, 3, h_los_ratio.substr(3, 3));
            }
        } else {
            if (to_int(h_los_ratio) > 0 && to_int(VAR_177) > 0) {
                if (h_los_ratio.substr(3, 3) > h_los_ratio.substr(0, 3)) {
                    VAR_195.replace(103, 3, h_los_ratio.substr(0, 3));
                    VAR_195.replace(106, 3, std::string(3, '0'));
                } else {
...
void p_1700_edit_drg_code() {
    VAR_195.replace(95, 3, VAR_177.substr(18, 3));
    if (to_int(VAR_195) == 0) {
        while (true) {
            ...
            if (false /* TODO: (ret = memcmp (VAR_25 + 11LL * ((cob_s64_t)(wwm_indx) - 1), VAR_195 + 95, 3)) == 0 */) {
                p_1750_find_value();
                break;
            }
            // if (ret < 0) head = b_158;
...
void p_2000_assemble_pps_variables() {
    if (false /* cob_cmp >= 0 */) {
        if (VAR_177.substr(29, 8) >= VAR_234.substr(24, 8)) {
            if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_332, VAR_327 + 25)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_332, VAR_327 + 25), 0LL) > 0 */) {
                // MOVE to computed field
            } else {
                VAR_195 = "52";
            }
        } else {
            if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_331, VAR_327 + 19)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_331, VAR_327 + 19), 0LL) > 0 */) {
                // MOVE to computed field
            } else {
                VAR_195 = "52";
            }
        ...
        if (VAR_177.substr(29, 8) >= VAR_234.substr(24, 8)) {
            ...
            if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_330, VAR_327 + 13)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_330, VAR_327 + 13), 0LL) > 0 */) {
                // MOVE to computed field
            } else {
                VAR_195 = "52";
            }
    ...
    if (VAR_234.at(137) == '1') {
        if (false /* TODO: cob_is_numeric (COB_SET_DATA (f_310, VAR_234 + 154)) */ && false /* TODO: cob_cmp_llint (COB_SET_DATA (f_310, VAR_234 + 154), 0LL) > 0 */) {
            VAR_195.replace(15, 6, VAR_234.substr(154, 6));
        } else {
            VAR_195 = "52";
        }
    }
    if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_291, VAR_234 + 101)) */) {
        VAR_195 = "65";
    }
    // MOVE to computed field
    if (to_int(VAR_195) > 0 && to_int(VAR_195) < 6) {
    } else {
        VAR_195 = "72";
    }
    h_blend_fac = "0";
...
void p_3000_calc_payment() {
    VAR_195.replace(110, 4, VAR_234.substr(87, 4));
...
void p_3400_short_stay() {
    ...
    if (h_los_ratio.substr(28, 9) < h_los_ratio.substr(19, 9)) {
        if (h_los_ratio.substr(28, 9) < VAR_195.substr(41, 9)) {
            VAR_195.replace(41, 9, h_los_ratio.substr(28, 9));
            VAR_195 = "02";
        } else {
        }
    } else {
        if (h_los_ratio.substr(19, 9) < VAR_195.substr(41, 9)) {
            VAR_195.replace(41, 9, h_los_ratio.substr(19, 9));
            VAR_195 = "02";
        } else {
        }
    }
...
void p_4000_special_provider() {
    if (VAR_177.substr(29, 8) >= "20030701" && VAR_177.substr(29, 8) < "20040101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    }
    if (VAR_177.substr(29, 8) >= "20040101" && VAR_177.substr(29, 8) < "20050101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    }
    if (VAR_177.substr(29, 8) >= "20050101" && VAR_177.substr(29, 8) < "20060101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    }
    if (VAR_177.substr(29, 8) >= "20060101" && VAR_177.substr(29, 8) < "20070101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    }
    if (VAR_177.substr(29, 8) >= "20070101") {
        h_ss_cost = to_string(to_num(0) * to_num(1));
        h_ss_pay_amt = to_string(to_num(0) * to_num(1));
    }
}
...
void p_7000_calc_outlier() {
    if (VAR_195.substr(68, 9) > VAR_195.substr(86, 9)) {
    }
    if (VAR_177.at(46) == '1') {
        // MOVE "0" to computed field
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_203, VAR_195 + 29), 0LL) > 0 */ && to_int(VAR_195) == 2) {
        VAR_195 = "03";
    }
    if (false /* TODO: cob_cmp_llint (COB_SET_DATA (f_203, VAR_195 + 29), 0LL) > 0 */ && to_int(VAR_195) == 0) {
        VAR_195 = "01";
    }
    if (to_int(VAR_195) == 0 || to_int(VAR_195) == 2) {
        if (false /* TODO: cob_cmp_numdisp (VAR_195 + 103, 3, cob_get_llint (h_ssot), 0) > 0 */) {
            VAR_195.replace(106, 3, std::string(3, '0'));
        } else {
        }
    }
    if (to_int(VAR_195) == 1 || to_int(VAR_195) == 3) {
        if (VAR_177.substr(24, 3) < h_los_ratio.substr(0, 3) || VAR_195.at(161) == 'Y') {
            VAR_195 = "67";
        } else {
        }
    } else {
...
void p_9000_move_results() {
    if (to_int(VAR_195) < 50) {
        VAR_195.replace(38, 3, h_los_ratio.substr(0, 3));
        VAR_195.replace(98, 5, cal_version.substr(0, 5));
    } else {
        VAR_195.replace(11, 4, std::string(4, ' '));
        VAR_195.replace(15, 80, std::string(80, '0'));
        VAR_195.replace(95, 8, std::string(8, ' '));
