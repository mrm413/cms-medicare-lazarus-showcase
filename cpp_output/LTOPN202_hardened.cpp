...
std::string VAR_428; // Auto-declared by LAZARUS healer
...
FixedString<50> cbsax_cbsa;
FixedString<50> cbsax_stat;
FixedString<50> cbsax_table_from_user;
FixedString<50> cbsa_cnt;
FixedString<50> cu1;
FixedString<50> cu2;
FixedString<50> eof_sw;
FixedString<8> idx_diag;
FixedString<8> idx_proc;
FixedString<50> ipps_cbsax_stat;
FixedString<50> ipps_cbsax_table_from_user;
FixedString<50> ipps_cbsa_cnt;
FixedString<50> ltdrv202;
FixedString<50> ma1;
FixedString<50> ma2;
FixedString<50> ma3;
FixedString<50> msax_msa;
FixedString<50> msax_stat;
FixedString<50> msax_table_from_user;
FixedString<50> msa_cnt;
FixedString<50> mu1;
FixedString<50> mu2;
FixedString<50> opn_version;
FixedString<50> pd2;
FixedString<50> pd3;
...
FixedString<50> prov_cnt;
FixedString<80> prov_data_2;
FixedString<80> prov_data_3;
FixedString<50> prov_new_hold;
FixedString<50> prov_stat;
FixedString<50> prov_table;
FixedString<50> px1;
int RETURN_CODE = 0;
FixedString<50> tables_loaded_sw;
FixedString<50> t_cbsa;
FixedString<50> ut_s_ipcbsax;
FixedString<50> ut_s_ppscbsax;
FixedString<50> ut_s_ppsmsax;
FixedString<50> ut_s_ppsprov;
FixedString<10> work_counters;
...
void p_0000_test_pricer_option_sw();
void p_1200_get_this_provider();
void p_1200_exit();
void p_1300_get_curr_prov();
void p_1300_exit();
void p_1500_load_all_tables();
void p_1500_exit();
void p_1600_read_prov_file();
void p_1600_exit();
void p_1700_load_msax_file();
void p_1700_exit();
void p_1750_load_cbsax_file();
void p_1750_exit();
void p_1775_load_ipps_cbsax_file();
void p_1775_exit();
void p_1800_read_msax_file();
void p_1800_exit();
void p_1850_read_cbsax_file();
void p_1850_exit();
void p_1875_read_ipps_cbsax_file();
void p_1875_exit();
void p_1900_option_sw_a();
void p_1900_exit();
void p_2000_option_sw_p();
void p_2000_exit();
void p_2100_option_sw();
void p_2100_exit();
...
void p_0000_test_pricer_option_sw() {
    ...
    if (pricer_opt_vers_sw.at(0) == 'A') {
        p_1900_option_sw_a();
    } else {
        if (pricer_opt_vers_sw.at(0) == 'P') {
            p_2000_option_sw_p();
        } else {
            p_2100_option_sw();
        }
    }
    if (pricer_opt_vers_sw.at(0) == 'P' || pricer_opt_vers_sw.at(0) == 'A') {
        pps_data_all = std::string(2, '0');
    } else {
        p_1200_get_this_provider();
    }
    if (pps_data_all.substr(0, 2) == "59") {
    }
}

void p_1200_get_this_provider() {
    px1 = to_string(1);
    // UNHANDLED: const int max = cob_get_numdisp (b_225 + 10, 5);
    while (true) {
        if (false /* TODO: px1 > max */) {
            pps_data_all = "59";
        }
        if (false /* TODO: memcmp (prov_table + 10 + 80LL * ((cob_s64_t)(px1) - 1), bill_new_data + 10, 6) == 0 */) {
            pps_data_all = std::string(2, '0');
            // pd2 = b_232;
            // pd3 = b_232;
            // px1 = b_232;
            while (true) {
                if (false /* TODO: memcmp (prov_table + 10 + 80LL * ((cob_s64_t)(px1) - 1), bill_new_data + 10, 6) != 0 */ || false /* TODO: memcmp (prov_table + 10 + 80LL * ((cob_s64_t)(px1) - 1), (cob_u8_ptr)"999999", 6) == 0 */) break;
                p_1300_get_curr_prov();
                // px1 = (b_232 + 1);
            }
...
void p_1300_get_curr_prov() {
    if (false /* TODO: memcmp (bill_new_data + 32, prov_table + 16 + 80LL * ((cob_s64_t)(px1) - 1), 8) >= 0 */) {
        // pd2 = b_232;
        // pd3 = b_232;
    }
}
...
void p_1500_load_all_tables() {
    prov_new_hold = std::string(240, static_cast<char>(57));
    // OPEN PROV-FILE (INPUT)
    if (file_status != "EXCEPTION_0") {
    }
    eof_sw.at(0) = '0';
    // px1 = cob_get_numdisp (b_87, 1);
    while (true) {
        if (to_int(eof_sw) == 1) break;
        p_1600_read_prov_file();
    }
    // CLOSE PROV-FILE
    if (file_status != "EXCEPTION_0") {
    }
    p_1700_load_msax_file();
    p_1750_load_cbsax_file();
    p_1775_load_ipps_cbsax_file();
}
...
void p_1600_read_prov_file() {
    ...
    if (to_int(eof_sw) == 0) {
        prov_cnt = to_string(to_int(prov_cnt) + 1);
        // px1 = (b_232 + 1);
        // pd2 = b_232;
        // pd3 = b_232;
    }
...
void p_1700_load_msax_file() {
    // OPEN MSAX-FILE (INPUT)
    if (file_status != "EXCEPTION_0") {
    }
    eof_sw.at(0) = '0';
    // mu1 = cob_get_numdisp (b_87, 1);
    while (true) {
        if (to_int(eof_sw) == 1) break;
        p_1800_read_msax_file();
    }
    // CLOSE MSAX-FILE
    if (file_status != "EXCEPTION_0") {
    }
...
void p_1750_load_cbsax_file() {
    // OPEN CBSAX-FILE (INPUT)
    if (file_status != "EXCEPTION_0") {
    }
    eof_sw.at(0) = '0';
    // cu1 = cob_get_numdisp (b_87, 1);
    while (true) {
        if (to_int(eof_sw) == 1) break;
        p_1850_read_cbsax_file();
    }
    // CLOSE CBSAX-FILE
    if (file_status != "EXCEPTION_0") {
    }
...
void p_1775_load_ipps_cbsax_file() {
    // OPEN IPPS-CBSAX-FILE (INPUT)
    if (file_status != "EXCEPTION_0") {
    }
    eof_sw.at(0) = '0';
    // ma3 = cob_get_numdisp (b_87, 1);
    while (true) {
        if (to_int(eof_sw) == 1) break;
        p_1875_read_ipps_cbsax_file();
    }
...
void p_1800_read_msax_file() {
    ...
    if (to_int(eof_sw) == 0) {
        msa_cnt = to_string(to_int(msa_cnt) + 1);
        // mu1 = (b_218 + 1);
    }
}
...
void p_1850_read_cbsax_file() {
    ...
    if (to_int(eof_sw) == 0) {
        cbsa_cnt = to_string(to_int(cbsa_cnt) + 1);
        // cu1 = (b_199 + 1);
    }
}
...
void p_1875_read_ipps_cbsax_file() {
    ...
    if (to_int(eof_sw) == 0) {
        ipps_cbsa_cnt = to_string(to_int(ipps_cbsa_cnt) + 1);
        // ma3 = (b_210 + 1);
    }
}
...
void p_1900_option_sw_a() {
    prov_new_hold = std::string(240, static_cast<char>(57));
    prov_new_hold = VAR_428.substr(0, 240);
    if (to_int(tables_loaded_sw) == 0) {
        // MOVE to computed field
        // MOVE to computed field
        // MOVE to computed field
        tables_loaded_sw.at(0) = '1';
    }
}
