...
FixedString<80> bill_315_data;
FixedString<50> bill_bene_wage_index;
FixedString<50> bill_ctr;
FixedString<50> bill_from_yy;
...
FixedString<50> cu2;
FixedString<50> cu3;
FixedString<50> eof_bill_sw;
FixedString<50> eof_cbsa_sw;
FixedString<50> eof_msa_sw;
FixedString<50> eof_prov_sw;
FixedString<50> hosop210;
FixedString<50> hospr210;
...
FixedString<50> pd2;
FixedString<50> pd3;
FixedString<50> prov_ctr;
FixedString<50> px1;
FixedString<50> px2;
FixedString<50> px3;
FixedString<50> p_new_geo_rural2;
FixedString<50> p_new_npi10;
FixedString<50> rate_ctr;
int RETURN_CODE = 0;
FixedString<50> search_cbsa;
FixedString<50> search_cbsa_pos45;
FixedString<50> search_msa;
FixedString<50> search_msa_pos34;
FixedString<50> ut1_stat;
FixedString<50> ut2_stat;
FixedString<50> ut3_stat;
FixedString<50> ut4_stat;
FixedString<50> ut5_stat;
FixedString<50> w_fy_begin_cc;
FixedString<50> w_fy_begin_yy;
FixedString<50> w_fy_end_cc;
FixedString<50> w_fy_end_yy;
FixedString<50> w_p_new_cbsa_geo_rural2nd;
...
void p_0200_process_records();
void p_0200_exit();
void p_0300_process_data();
void p_0300_exit();
void p_0350_get_msa();
void p_0350_exit();
void p_0375_get_cbsa();
void p_0375_exit();
void p_0400_search_4_msa();
void p_0400_search_exit();
void p_0450_search_4_cbsa();
void p_0450_search_exit();
void p_0500_get_hosp_wage_index();
void p_0500_exit();
void p_0525_get_hosp_wage_index();
void p_0525_exit();
void p_0550_get_bene_wage_index();
void p_0550_exit();
void p_0575_get_bene_wage_index();
void p_0575_exit();
void p_0700_get_provider();
void p_0700_exit();
void p_0800_get_curr_prov();
void p_0800_exit();
void p_1000_call();
void p_1000_exit();

void p_0200_process_records() {
    p_0200_process_records();
    bill_315_data.replace(293, 10, std::string(10, '0'));
    // MOVE "0" to computed field
    // MOVE "0" to computed field
    // MOVE "0" to computed field
    // MOVE "0" to computed field
    bill_315_data.replace(221, 72, std::string(72, '0'));
    ...
    w_fy_begin_yy = bill_315_data.substr(16, 2);
    w_fy_end_yy = bill_315_data.substr(16, 2);
    if (to_int(bill_315_data) >= 1 && to_int(bill_315_data) <= 9) {
        w_fy_begin_yy = to_string(to_num(0) - to_num(1));
        w_fy_end_yy.replace(2, 2, bill_315_data.substr(18, 2));
    } else {
        w_fy_begin_yy.replace(2, 2, bill_315_data.substr(18, 2));
        w_fy_end_yy = to_string(to_num(0) + to_num(1));
    }
    if (to_int(eof_bill_sw) == 0) {
        bill_ctr = to_string(to_int(bill_ctr) + 1);
        p_0300_process_data();
    }
}
...
void p_0300_process_data() {
    p_0700_get_provider();
    if (bill_315_data.substr(301, 2) != "00") {
    }
    if (w_p_new_cbsa_geo_rural2nd.substr(16, 8) < "20051001" && bill_315_data.substr(16, 8) > "20050930") {
        bill_315_data.replace(301, 2, "51");
    }
    if (bill_315_data.substr(16, 8) > "20050930") {
        p_0375_get_cbsa();
    } else {
        p_0350_get_msa();
    }
...
void p_0350_get_msa() {
    if (w_p_new_cbsa_geo_rural2nd.substr(58, 2) == "") {
        search_msa_pos34 = std::string(2, static_cast<char>(57));
        search_msa_pos34 = p_new_geo_rural2;
    } else {
        search_msa_pos34 = w_p_new_cbsa_geo_rural2nd.substr(58, 4);
    }
    if (bill_315_data.substr(16, 8) < "19991001") {
        search_msa_pos34.at(4) = w_p_new_cbsa_geo_rural2nd.at(72);
    } else {
        search_msa_pos34.at(4) = ' ';
    }
    bill_315_data.replace(42, 4, w_p_new_cbsa_geo_rural2nd.substr(58, 4));
    if (bill_315_data.substr(16, 8) < "19991001") {
        bill_315_data.at(46) = w_p_new_cbsa_geo_rural2nd.at(72);
    } else {
        bill_315_data.at(46) = ' ';
    }
    if (bill_315_data.substr(16, 8) < "19991001") {
    ...
    if (bill_315_data.substr(301, 2) == "00") {
        // mu2 = b_304;
        while (true) {
            if (false /* TODO: memcmp (VAR_302 + 19LL * ((cob_s64_t)(mu2) - 1), search_msa_pos34, 5) != 0 */) break;
            p_0500_get_hosp_wage_index();
            // mu2 = (b_305 + 1);
        }
    } else {
        // MOVE "0" to computed field
    ...
    if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_211, bill_315_data + 52)) */ || false /* TODO: cob_cmp_llint (COB_SET_DATA (f_211, bill_315_data + 52), 0LL) == 0 */) {
        bill_315_data.replace(301, 2, "40");
    }
    search_msa_pos34 = bill_315_data.substr(47, 4);
    if (bill_315_data.substr(16, 8) < "19991001") {
        search_msa_pos34.at(4) = bill_315_data.at(51);
    } else {
        search_msa_pos34.at(4) = ' ';
    }
    p_0400_search_4_msa();
    ...
    if (false /* TODO: !cob_is_numeric (COB_SET_DATA (f_212, bill_315_data + 58)) */ || false /* TODO: cob_cmp_llint (COB_SET_DATA (f_212, bill_315_data + 58), 0LL) == 0 */) {
        bill_315_data.replace(301, 2, "50");
    }
}
...
void p_0375_get_cbsa() {
    if (bill_315_data.substr(16, 8) < "20080101") {
        if (w_p_new_cbsa_geo_rural2nd.substr(139, 3) == "") {
            search_cbsa_pos45 = std::string(3, static_cast<char>(57));
            search_cbsa_pos45 = w_p_new_cbsa_geo_rural2nd;
        } else {
            search_cbsa_pos45 = w_p_new_cbsa_geo_rural2nd.substr(139, 5);
            bill_315_data.replace(42, 5, w_p_new_cbsa_geo_rural2nd.substr(139, 5));
        }
    } else {
        search_cbsa_pos45 = bill_315_data.substr(42, 5);
    }
    p_0450_search_4_cbsa();
    if (bill_315_data.substr(301, 2) == "00") {
        // cu2 = b_314;
        while (true) {
            if (false /* cob_cmp != 0 */) break;
            p_0525_get_hosp_wage_index();
            // cu2 = (b_315 + 1);
        }
    } else {
        // MOVE "0" to computed field
    ...
    search_cbsa_pos45 = bill_315_data.substr(47, 5);
    p_0450_search_4_cbsa();
    if (bill_315_data.substr(301, 2) == "00") {
        // cu2 = b_314;
        while (true) {
            if (false /* cob_cmp != 0 */) break;
            p_0575_get_bene_wage_index();
            // cu2 = (b_315 + 1);
        }
    } else {
        // MOVE "0" to computed field
...
void p_0400_search_4_msa() {
    mu1 = to_string(1);
    // UNHANDLED: const int max = 4000;
    while (true) {
        if (false /* TODO: mu1 > max */) {
            bill_315_data.replace(301, 2, "30");
            break;
        }
        if (false /* TODO: memcmp (VAR_302 + 19LL * ((cob_s64_t)(mu1) - 1), search_msa_pos34, 5) == 0 */) {
            // mu2 = b_304;
            break;
        }
        // UNHANDLED: b_304++;
...
void p_0500_get_hosp_wage_index() {
    if (false /* TODO: memcmp (bill_315_data + 16, VAR_302 + 5 + 19LL * ((cob_s64_t)(mu2) - 1), 8) >= 0 */) {
...
void p_0525_get_hosp_wage_index() {
    if (false /* TODO: memcmp (bill_315_data + 16, VAR_312 + 5 + 19LL * ((cob_s64_t)(cu2) - 1), 8) >= 0 */ && false /* TODO: memcmp (VAR_312 + 5 + 19LL * ((cob_s64_t)(cu2) - 1), w_fy_begin_yy, 8) >= 0 */ && false /* TODO: memcmp (VAR_312 + 5 + 19LL * ((cob_s64_t)(cu2) - 1), w_fy_end_yy, 8) <= 0 */) {
        // MOVE to computed field
    }
}
...
void p_0700_get_provider() {
    if (bill_315_data.substr(10, 6) != w_p_new_cbsa_geo_rural2nd.substr(10, 6)) {
        px2 = to_string(1);
        // UNHANDLED: const int max = 2400;
        while (true) {
            if (false /* TODO: px2 > max */) {
                bill_315_data.replace(301, 2, "51");
            }
            if (false /* TODO: memcmp (bill_315_data + 10, VAR_282 + 10 + 80LL * ((cob_s64_t)(px2) - 1), 6) == 0 */) {
                bill_315_data.replace(301, 2, std::string(2, '0'));
                break;
            }
            // UNHANDLED: b_285++;
        }
    ...
    // px3 = b_285;
    while (true) {
        if (false /* TODO: memcmp (VAR_282 + 10 + 80LL * ((cob_s64_t)(px3) - 1), bill_315_data + 10, 6) != 0 */ || false /* TODO: memcmp (VAR_282 + 10 + 80LL * ((cob_s64_t)(px3) - 1), (cob_u8_ptr)"999999", 6) == 0 */) break;
        p_0800_get_curr_prov();
        // px3 = (b_286 + 1);
    }
}
...
void p_0800_get_curr_prov() {
    if (false /* TODO: memcmp (bill_315_data + 16, VAR_282 + 16 + 80LL * ((cob_s64_t)(px3) - 1), 8) >= 0 */) {
        // pd2 = b_286;
        // pd3 = b_286;
    }
}
